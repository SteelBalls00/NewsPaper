...
def censor(content, censor_list):
    ...
    for i in range(len(split_content)):                                # Каждое слово из текста приводится в нижний регистр
        if split_content[i].lower() in lower_censor_list:              # и сравнивается с цензор-списком.
